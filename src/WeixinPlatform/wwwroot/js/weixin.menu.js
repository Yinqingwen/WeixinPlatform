...
weixin.menu = {
    ...
    init: function () {
        ...
        $("#buttonDetails_type").change(weixin.menu.typeChanged);
        ...
        $('#menuLogin_Submit').click(function () {
            $.getJSON('/Menu/GetToken?t=' + Math.random(), { appId: $('#inputAppId').val(), appSecret: $('#inputAppSecret').val() },
                function (json) {
                    if (json.access_token) {
                        weixin.menu.setToken(json.access_token);
                    } else {
        ...
        $('#menuLogin_SubmitOldToken').click(function () {
            alert($("#inputToken").val());
            weixin.menu.setToken($('#inputToken').val());
        });

        $('#btnGetMenu').click(function () {
            menuState.html('获取菜单中...');
            $.getJSON('/Menu/GetMenu?t=' + Math.random(), { token: weixin.menu.token }, function (json) {
                if (json.menu) {
        ...
        $('#btnDeleteMenu').click(function () {
            ...
            $.getJSON('/Menu/DeleteMenu?t=' + Math.random(), { token: weixin.menu.token }, function (json) {
                if (json.Success) {
