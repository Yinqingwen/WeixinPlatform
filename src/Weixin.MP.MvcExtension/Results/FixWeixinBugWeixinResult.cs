    ...
    public class FixWeixinBugWeixinResult : ContentResult
    {
        ...
        /*public override void ExecuteResult(ControllerContext context)
        {
            if (base.Content == null)
            {
                ...
                {
                    throw new Weixin.Exceptions.WeixinException("执行WeixinResult时提供的MessageHandler不能为Null！", null);
                }

                if (_messageHandlerDocument.FinalResponseDocument == null)
                {
                    //throw new Weixin.MP.WeixinException("FinalResponseDocument不能为Null！", null);
                }
                else
                {
