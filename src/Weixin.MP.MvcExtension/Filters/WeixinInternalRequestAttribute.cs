...
using System;
using Weixin.BrowserUtility;

namespace Weixin.MP.MvcExtension
{
    /// <summary>
    /// 过滤来自非微信客户端浏览器的请求
    /// </summary>
    ...
    public class WeixinInternalRequestAttribute : ActionFilterAttribute
    {
        ...
        {
            if (string.IsNullOrEmpty(_ignoreParameter))// || string.IsNullOrEmpty(filterContext.HttpContext.Request.QueryString[_ignoreParameter]).ToString())
            {
                if (true)//!BroswerUtility.SideInWeixinBroswer(filterContext.HttpContext))
                {
                    //TODO:判断网页版登陆状态
